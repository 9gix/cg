...
const GLfloat light0ReflectionPosition[] = { light0Position[0], light0Position[1], light0Position[2] - TABLETOP_Z, light0Position[3] };
...
const GLfloat light1ReflectionPosition[] = { light1Position[0], light1Position[1], light1Position[2] - TABLETOP_Z, light1Position[3] };
...
const char specialTexFile[] = "images/abstract.png";
...
void MakeReflectionImage( void )
{
    ...
    glLightfv(GL_LIGHT0, GL_POSITION, light0ReflectionPosition);
    glLightfv(GL_LIGHT1, GL_POSITION, light1ReflectionPosition);
...
// This texture object is for the special object
    glGenTextures(1, &specialTexObj);
    glBindTexture(GL_TEXTURE_2D, specialTexObj);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);

    if (ReadImageFile(specialTexFile, &imageData,
        &imageWidth, &imageHeight, &numComponents) == 0) exit(1);
    if (numComponents != 3) {
        fprintf(stderr, "Error: Texture image is not in RGB format.\n");
        exit(1);
...
void DrawSomething(void)
{
    ...
    glBindTexture(GL_TEXTURE_2D, specialTexObj);
    glPushMatrix();
    glTranslated(0.9, -0.5, TABLETOP_Z + 0.3);
    DrawTube();
    glPopMatrix();
}

void DrawTube(void) {
    glDisable(GL_CULL_FACE);  // Disable back-face culling for tube object.
    const double radius = 0.5 / PI;
    const int numSectors = 100;
    const double angleIncr = 2.0 * PI / numSectors;
    const double texCoordIncr = 0.5 / numSectors;
